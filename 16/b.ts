...
const explore = (startdId = "AA", time: number, ignores: string[] = []) => {
  const unexplored = visit(
    startdId,
    tunnelGraph.filter((v) => v.flowRate > 0 && !ignores.includes(v.valve)).map(
      (m) => m.valve,
    ),
  );
  const all = [];
  ...
  return [sorted[0]?.score ?? 0, sorted] as [
    number,
...
let secondStar = 0, i = 0;
const count = humanPaths.length;
